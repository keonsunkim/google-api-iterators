...
class CSVSaveMixin(BaseSaveMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.csv_file_directory = kwargs.get('csv_file_directory')
        
        super(CSVSaveMixin, self).__init__(*args, **kwargs)
        
        # This part became unneccessarily repetitive. Work on it later
        try:
            self.data_file = open(f'{self.csv_file_directory}', 'r', encoding = 'utf-8')
            self.data_file.close()
        except:
            self.data_file = open(f'{self.csv_file_directory}', 'w+', encoding = 'utf-8')
            self.csv_dict_writer = csv.DictWriter(self.data_file, fieldnames = self.header)
            self.csv_dict_writer.writeheader()
            self.data_file.close()
        finally:
            self.data_file = open(f'{self.csv_file_directory}', 'a', encoding = 'utf-8')
            self.csv_dict_writer = csv.DictWriter(self.data_file, fieldnames = self.header)
    ...
    def save(self, *args, **kwargs):
        ...
        list_to_save = self._parser_function()
